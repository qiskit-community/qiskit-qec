...
TODO:
    ...
    - Allow for empty PauliList objects
    - Separate the symplectic representation to an new class and allow for other 
    types of representations (needed for large LDPC codes etc when dimensions are large)
"""
...
Change Log:
 ...
 Jul 27, 2021 Drew Vandeth
   ...
   Modified to allow the creation of a PauliList from a list of Paulis with
   different num_qubits. So now you can say a = PauliList(['X','ZY']) and get
   the equivalent form PauliList(['IX', 'ZY'])

   Changes to Pauli class now allow using two syntax formats for Pauli string
   inputs - Product syntax and Index syntax. See notes in Pauli class (basically
   allows for '-iXYXXIII' and '-iX3X4Y5X6' and reverse order qubit input)

Aus 9, 2021 Drew Vandeth
    Modified from_symplectic to allow x or z to be None. 
"""
...
class PauliList(PaulisBase, LinearMixin, GroupMixin):
    ...
    **Initialization**
    ...
    A PauliList obj can be initialized in several ways.
    ...
    **Iteration**
    ...
    def __init__(self, data):
        ...
        Args:
            data (Pauli or list): input data for Paulis. If input is a list each item in the list
                                  must be a Pauli obj or Pauli str.
    ...
    def _from_paulis(data):
        ...
        # Note that the next step implicitly broadcasts in input array from two different shapes
        # This is take extra time but is probably okay for most applications
        for i, pauli in enumerate(paulis):
            base_z[i][: qubit_count[i]] = pauli._z
            base_x[i][: qubit_count[i]] = pauli._x
    ...
    def equiv(self, other):
        ...
        Args:
            other (PauliList or Pauli): a comparison obj.
    ...
    def label_iter(self):
        ...
        Returns:
            LabelIterator: label iterator obj for the PauliList.
        """
    ...
    def matrix_iter(self, sparse=False):
        ...
        Returns:
            MatrixIterator: matrix iterator obj for the PauliList.
        """
    ...
    @classmethod
    def from_symplectic(cls, z=None, x=None, phase=0):
        """Construct a PauliList from a symplectic data.

        If either x or z are None then they will be assumed to be all
        False (0)

        Args:
            z (np.ndarray): 2D boolean Numpy array. Default = None
            x (np.ndarray): 2D boolean Numpy array. Defulat = None
        ...
        if z is None and x is None:
            # Create an empty PauliList. Need to implement an empty PauliList
            raise QiskitError("Empty PauliLists are not yet implemented")
        if x is None:
            z = np.asarray(z)
            x = np.zeros(z.shape)
        elif z is None:
