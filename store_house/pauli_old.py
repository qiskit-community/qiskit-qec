...
import numpy as np
import qiskit_qec.qec_utils.pauli_rep
...
# from qiskit.quantum_info.operators.sympl import TranslationMap
    ...
    *Pauli* group
    ...
    def use_class_map(self, identifier):
        ...
        if not self.map_is_registered_in_class(identifier):
            QiskitError(
                f"Class map with identifier {identifier} is not"
    ...
    def _register_class_map(self, *, mapper, identifier, override):
        if not override:
            if identifier in Pauli.__class_map_dict__.keys():
                QiskitError(
                    "Override is on. Mapper with same identifty in class map dictionary already"
    ...
    def _register_instance_map(self, *, mapper, identifier, override):
        if not override:
            if identifier in self.__instance_map_dict__.keys():
                QiskitError(
                    "Override is on. Mapper with same identifty in instance map dictionary already"
                    'registered. Set override="True" to force override'
                )
        self.__instance_map_dict__[identifier] = mapper

    def map_is_registered(self, identifier):
        return self.map_is_registered_in_class(identifier) or self.map_is_registered_in_instance(
            identifier
        )
