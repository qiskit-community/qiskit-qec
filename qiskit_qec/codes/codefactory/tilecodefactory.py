...
class TileCodeFactory:
    ...
    def make_code(self) -> StabSubSystemCode:
        ...
        if self.lattice_view:
            self.plot(
                lattice=lattice, cutter=self.cutter, cutter_color=self.cutter_color
            )
    ...
    def plot(
        ...
        if shell is not None:
            ...
            if show_inside:
                data_in = [
                    list(vertex.pos) for vertex in shell.vertices if is_inside[vertex]
                ]
                in_x = [v[0] for v in data_in]
                in_y = [v[1] for v in data_in]
