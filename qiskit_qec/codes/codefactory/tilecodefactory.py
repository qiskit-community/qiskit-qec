...
class TileCodeFactory:
    ...
    DRAWING_PARAMETERS      = ["cutter_color",
                               "show_qubit_indices",
                               "show_face_indices",
                               "show_face_colors",
                               "show_inside",
    ...
    PARAMETER_DICT = {"manifold" : MANIFOLD_PARAMETERS,
    ...
    def update_is_configure(self):
        ...
        for para_set in self.configured:
            self.configured[para_set] = all(getattr(self, parameter) is not None \
    ...
    def make_code(self)->StabSubSystemCode:
        ...
        if not self.is_configured():
            not_configured = {key:value for key, value in self.configured.items() if value is False}
            raise QiskitError(f"Factory is not fully configured: {not_configured}. If" \
        ...
        return StabSubSystemCode(shell=new_shell,
