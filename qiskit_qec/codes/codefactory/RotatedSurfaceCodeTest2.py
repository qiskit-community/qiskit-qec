...
def check_cell_arrays(grid):
    ...
    for idx in range(numArrays):
        ...
        if numTuples < numCells:
            ...
            print(
                f"Cell array {name} with indx ={idx} {array.GetNumberOfComponents()} components, only has {numTuples} tuples but there are {numCells} cells"
            )
            ...
            print(f"Array {name} failed")
            print(
                f"Cell array {name} with {array.GetNumberOfComponents()} components, has {numTuples} tuples but there are only {numCells}  cells"
            )
...
def check_point_arrays(grid):
    ...
    for idx in range(numArrays):
        ...
        if numTuples < numPoints:
            ...
            print(f"Array {name} failed")
            print(
                f"Point array {name} with indx ={idx} {array.GetNumberOfComponents()} components, only has {numTuples} tuples but there are {numPoints} points"
            )
            for k in range(array.GetNumberOfTuples()):
                print(f"array: {array.GetTuple(k)}")
            return 1
            ...
            print(f"Array {name} failed")
            print(
                f"Point array {name} with {array.GetNumberOfComponents()} components, has {numTuples} tuples but there are only {numPoints}  points"
            )
...
class RotatedSurfaceCodeGeometricModeler(GeometricCodeModelModeler):
    ...
    def setShape(self, shape_data, override=False):
        if self.shape_flag == self.SET:
            if override == False:
                raise (
                    "Shape of code has already been processed. "
    ...
    def setCellParity(self, cellParity):
        if cellParity not in [self.X, self.Z]:
            raise (
                "Unknown cell parity: Use one of <class RotatedSurfaceCodeModel>.X (=0) or <class RotatedSurfaceCodeModel>.Z (=1)"
            )
    ...
    def generateBoundaryPointIds(self):
        ...
        int_shape_boundary_points = make_int_entries(
            RotatedSurfaceCodeGeometricModeler.extractShapePointsArray(int_shape_boundary)
        )
            ...
            for y in range(ymax, ymin, -self.cellUnit):
                ...
                if self.getShapeMask().IsInsideSurface(center):
                    ...
                    for stab_point in stab_points:
                        ...
                        ids.append(stab_id)
                        self.grid.GetPointData().GetArray(self.point_type_name).InsertValue(
                            stab_id, self.DATA_QUBIT
                        )

                    ids = [
                        self.grid.GetPoints().InsertNextPoint(stab_point)
                        for stab_point in stab_points
                    ]
                    ...
                    if (x + y + self.cellParity) % 2 == 0:
                        ...
                        self.grid.GetCellData().GetArray(
                            self.cell_stabilizer_type_name
                        ).InsertValue(cell_id, self.X)
                        # print(type(self.grid.GetCellData().GetArray(self.cell_colors_name)))
                        self.grid.GetCellData().GetArray(self.cell_colors_name).InsertValue(
                            cell_id, self.colorX
                        )
        ...
        for k in range(n):
            ...
            if back_cell_id == front_cell_id:  # Outside Corner (going around anticlockwise)
                ...
                boundary_options.append(
                    (current_point_id, current_point_id, int(not type), self.CORNER)
                )
                boundary_options.append(
                    (current_point_id, next_point_id, type, self.OUTSIDE_CORNER)
                )

            elif self.cell_stabilizer_type.GetValue(
                back_cell_id
            ) == self.cell_stabilizer_type.GetValue(
                front_cell_id
            ):  # Inside corner
                type = self.cell_stabilizer_type.GetValue(front_cell_id)
    ...
    def generateBoundaryStabilizers(self):
        ...
        for k in range(n):
            ...
            if current_section[3] == self.CORNER:  # Checking corners points for changes
                ...
                if (
                    current_section[2] != self.boundary_types[k]
                ):  # A change in corner type is requested
        ...
        print(
            f"Number of color tuples is = {self.grid.GetCellData().GetArray(self.cell_colors_name).GetNumberOfTuples()}"
        )
        if (
            self.grid.GetNumberOfCells()
            != self.grid.GetCellData().GetArray(self.cell_colors_name).GetNumberOfTuples()
        ):
        ...
        for k in range(len(partitioned_deleted_qubits_cell_ids)):
            ...
            if length == 2:
                ...
                self.grid.GetPointData().GetArray(self.point_type_name).InsertValue(
                    cp_id, self.NON_QUBIT
                )

                cid = self.grid.InsertNextCell(
                    vtk.VTK_TRIANGLE, 3, [new_cell_point_ids[0], new_cell_point_ids[1], cp_id]
                )

                cell_type = (
                    self.grid.GetCellData()
                    .GetArray(self.cell_stabilizer_type_name)
                    .GetValue(cell_id)
                )
                self.grid.GetCellData().GetArray(self.cell_stabilizer_type_name).InsertValue(
                    cid, cell_type
                )

                cell_color = (
                    self.grid.GetCellData().GetArray(self.cell_colors_name).GetValue(cell_id)
                )
                ...
                cid = self.grid.InsertNextCell(
                    vtk.VTK_TRIANGLE,
                    3,
                    [new_cell_point_ids[0], new_cell_point_ids[1], new_cell_point_ids[2]],
                )
        ...
        for k in range(n):
            ...
            if front_section[3] == self.INSIDE_CORNER:
                ...
                if back_type == front_section[2] and front_type == back_section[2]:
                    print(
                        f"Illegal boundary change at {previous_point_id} -> {current_point_id} -> {next_point_id}"
                    )
                    print(
                        f"{current_point_id} occurs at coordinates {self.grid.GetPoint(current_point_id)}"
                    )
                ...
                cell_stabilizer_type = self.grid.GetCellData().GetArray(
                    self.cell_stabilizer_type_name
                )
                ...
                if boundary_options[(k + 1) % n][2] != front_type:
                    ...
                    cid = self.grid.InsertNextCell(
                        vtk.VTK_TRIANGLE, 3, [current_point_id, next_point_id, pt_id]
                    )
        ...
        for k in range(n):
            ...
            print(
                f"Working with cell_id={cell_id} and indices {qubit_indices} of cell type {cell_type}"
            )
...
def main():
    ...
    if code == 2:
        ...
        full_boundary = [
            ...
            "X",
            [14, 2],
            "Z",  # Illegal changing boundaries [14,1]-[14,2] Z -> X combined with [14,2]-[14,3] X -> Z
            [14, 4],
            "X",
            [15, 8],
            "Z",
        ...
        full_boundary = [
            ...
            "X",
            [14, 2],
            "X",  # Illegal changing boundaries [14,1]-[14,2] Z -> X combined with [14,2]-[14,3] X -> Z
            [14, 4],
            "X",
            [15, 8],
            "Z",
