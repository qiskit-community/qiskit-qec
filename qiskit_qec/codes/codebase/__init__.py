...
class CodeLibrary:
    ...
    def __init__(self, name: str, path: str, config_filename: str) -> None:
        ...
        try:
            self.n_k_codes_file_format = _library_config["File Formats"][
            ...
            for n in range(self.n_min, self.n_max + 1, 1):
                ...
                for k in range(self.k_min, self.k_max + 1, 1):
                    ...
                    if os.path.exists(file_to_load):
                        with open(
                            file_to_load, mode="r", encoding="utf-8"
                        ) as codes_json_file:
                            self.data[n][k] = {
                                int(index): data
            ...
            for n, k in product(n_range, k_range):
                try:
                    ...
                    if index_range is not None:
                        data = {
                            ind: data[ind] for ind in index_range if ind in data.keys()
                        }
...
class CodeLibrarian:
    ...
    def __init__(self, config: Optional[str] = None) -> None:
        ...
        default_config = os.path.join(
            CodeLibrarian._codebase_path, CodeLibrarian._CONFIG_FILE
        )
        ...
        **kwargs,
        ...
        for library in self.libraries.values():
            if library.in_range(n, k, index):
                codes = codes + library.search(
                    n, k, index, info_only=info_only, **kwargs
                )
...
def small_code(
    n: int, k: int, index: int, info_only: bool = False
    ...
    if not isinstance(index, int):
        raise QiskitError(
            "index, index assigned to the requested [[n,k]] code, must be specified."
        )
    ...
    **kwargs,
    ...
    if index is not None:
        if not isinstance(index, (int, list)):
            raise QiskitError(
                f"index must be an integer or a list of integers: {index}"
            )
