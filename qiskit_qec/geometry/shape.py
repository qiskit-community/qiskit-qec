...
class Shape:
    ...
    def __init__(
        self, points: List, lines: Optional[List[List]] = None, indices: List = None
    ):
        """Init shape
        ...
        if lines is None:
            if lines is None and indices is not None:
                raise QiskitError(
                    "If <indices> are defined then lines must also be defined"
                )
    ...
    def rect(
        ...
        if scale1 < 0 or scale2 < 0:
            raise QiskitError(
                f"Rectangle scales must be positive: scale1:{scale1} scale2:{scale2}"
            )
    ...
    @staticmethod
    def is_between(
        p: Real, a: Real, b: Real, strict: bool = False, epsilon: Real = 0.0
    ):
    ...
    def contains(self, point, on_boundary=True, epsilon=0.01, method="winding") -> bool:
        ...
        if method == "winding":
            ...
            return self.contains_ray_trace(
                point, on_boundary=on_boundary, epsilon=epsilon
            )
        else:
    ...
    def contains_ray_trace(self, point, on_boundary=True, epsilon=0.01):
        ...
        for index, line in enumerate(self.lines):
            ...
            if m == float("inf"):
                if (
                    Shape.is_between(y, y0, y1, epsilon=0.000000001, strict=True)
                    and x < x0
                ):
                    count += 1
                ...
                if (
                    self.is_between(y, y0, y1, epsilon=0.000000001, strict=True)
                    and y < yp
                ):
                    count += 1
                    dprint(f"m<0 inside count = {count}")
    ...
    def inside(
        ...
        for vertex in tiling.vertices:
            ...
            inout[vertex] = self.contains(
                vertex.pos, on_boundary=on_boundary, epsilon=epsilon
            )
        return inout
