...
class Shell(ShapeObject):
    ...
    def extract(
        ...
        if exclude is None:
            ...
            def exclude(
                vertex_paths: List[List[Vertex]], qubit_data: QubitData
            ) -> bool:
                return False
        ...
        for face in self.faces:
                    ...
                    for path in vertex_paths:
                        ...
                        if len(path) > 1:
                            if path[0] == path[-1]:
                                ...
                                new_path = [
                                    vertex.shallowcopy() for vertex in path[:-1]
                                ]
                                short_path = path[:-1]
                                if debug:
                        ...
                        for new_vertex, old_vertex in zip(new_path, short_path):
                            ...
                            new_qubit_data.qubit[new_vertex.id] = qubit_data.qubit[
                                old_vertex.id
                            ]

                            # Record the use of the qubit
                            qubit_count.increment_qubit(
                                new_qubit_data.qubit[new_vertex.id]
                            )

                            # Set the vertex operator
                            new_qubit_data.operator[
                                new_vertex.id
                            ] = qubit_data.operator[old_vertex.id]
            ...
            if len(edges) > 0:
                ...
                try:
                    new_qubit_data.face_colors[new_face.id] = qubit_data.face_colors[
                        face.id
                    ]
                except KeyError:
                    pass
                ...
                try:
                    new_qubit_data.orientation[new_face.id] = qubit_data.orientation[
                        face.id
                    ]
    ...
    def shell2symplectic(
        ...
        if from_index is None:
            from_index = [
                qubit_id
                for qubit_id, count in qubit_count.qubits_count.items()
                if count != 0
            ]
        ...
        qubit_data.index = {
            vertex.id: from_qubit[qubit_data.qubit[vertex.id]]
        ...
        **kwargs,
        ...
        for stabilizer in stab:
            if len(stabilizer[0]) == 2:
                ax.add_patch(
                    plt.Polygon(stabilizer[0], color=stabilizer[1], linewidth=7)
                )
            else:
                ax.add_patch(
                    plt.Polygon(
                        stabilizer[0], facecolor=stabilizer[1], edgecolor="black"
                    )
                )
