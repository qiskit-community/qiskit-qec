...
class SymplecticMatrixDenseNumPy(SymplecticMatrixBase):
    ...
    def logical_and(x1, x2, **kwargs):
        return SymplecticMatrixDenseNumPy(xz_matrix=np.logical_and(x1._matrix, x2._matrix, **kwargs))

    
    @staticmethod
    def logical_xor(x1, x2, **kwargs):
        return SymplecticMatrixDenseNumPy(xz_matrix=np.logical_xor(x1._matrix, x2._matrix, **kwargs))
    ...
    def istack(matrix, size, interleave=False):
        ...
        if interleave:
            return np.hstack(size * [matrix._matrix]).reshape((size * len(matrix._matrix),) + matrix._matrix.shape[1:])

        return np.vstack(size * [matrix._matrix]).reshape((size * len(matrix._matrix),) + matrix._matrix.shape[1:])
