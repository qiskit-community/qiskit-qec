    ...
    def leftButtonPressEvent(self, obj, event):
        ...
        if self.NewPickedActor:
            ...
            if self.LastPickedActor:
                print(f"lastpickedactor type: {type(self.LastPickedActor)} w qutype: {self.LastPickedActor.qutype}")
                self.LastPickedActor.GetProperty().DeepCopy(self.LastPickedProperty)
                if self.LastPickedActor.qutype == DATA:
                    self.LastPickedActor.GetProperty().SetColor(colors.GetColor3d('pink'))
                elif self.LastPickedActor.qutype == BDATA:
                    self.LastPickedActor.GetProperty().SetColor(colors.GetColor3d('green'))
