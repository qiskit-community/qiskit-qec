...
class QiskitGameEngine:
    def __init__(self, start, next_frame, L=8, active_screen=False):
        ...
        interface = []
        interface.append(
            widgets.HBox([self.screen.pixel[x, 0].button for x in range(L)] + [b, u, b, b, b, X, b])
        )
        interface.append(
            widgets.HBox([self.screen.pixel[x, 1].button for x in range(L)] + [l, b, r, b, Y, b, A])
        )
        interface.append(
            widgets.HBox([self.screen.pixel[x, 2].button for x in range(L)] + [b, d, b, b, b, B, b])
        )
