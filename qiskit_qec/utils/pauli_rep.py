...
INTERNAL_PAULI_REP_FORMAT = INTERNAL_PHASE_REP_FORMAT \
    + INTERNAL_SYMP_REP_FORMAT

DEFAULT_EXTERNAL_SYMP_FORMAT = 'YZX'
DEFAULT_EXTERNAL_PHASE_REP_FORMAT = '-i'
DEFAULT_EXTERNAL_PAULI_REP_FORMAT = DEFAULT_EXTERNAL_PHASE_REP_FORMAT \
    + DEFAULT_EXTERNAL_SYMP_FORMAT
...
PAULI_REP_FORMATS =['iXZ','iXZY','iZX','iYZX',
                           '-iXZ','-iXZY','-iZX','-iYZX',
...
def _syntax_type(string):
    ...
    if _is_pattern(string, PRODUCT_SYNTAX):
        ...
        QiskitError(f"Unknown Pauli syntax type: {string}")
...
def convert_phase_exp(
    ...
    if input_format == output_format:
        return phase
    if (input_format not in phase_formats()) \
         or (output_format not in phase_formats()):
...
def change_rep(
    ...
    if not isinstance(phase_exponent, (int, np.integer)) and not isinstance(phase_exponent, (tuple, np.ndarray, list)):
        raise QiskitError("The phase exponent must be an element in Z_4"
            " or a pair of elements in GF(2) x GF(2)"
            )
...
def _change_rep(
    ...
    if output_phase_format == 'i':
        ...
        phase_exponent  = np.asarray([_cal_phase(exp, marker) for exp, marker in zip(phase_exponent,res)])
    else:
...
def norm_phase_str(phase_string, same_type=True):
    if is_scalar(phase_string) and same_type:
        scalar = True
    else:
        scalar = False
...
def r_norm_phase_str(phase_string):
    return np.array([string.replace("+", "", 1).replace("1", "", 1).replace("j", "i", 1) for string in phase_string])

def is_scalar(object):
    """Deteremine if an object is a scalar. This is not fully robust in that it 
    does not take into account all possible ways to encode a scalar but it
    does work for lists, tuples, and np.ndarrays
    """
    return (isinstance(object, np.ndarray) and object.shape == ()) or (not isinstance(object, (list, tuple)))
...
def squeeze(array_, scalar=False):
    ...
    if array_.shape == () and scalar==True:
        return array_.item()
    else:
        return array_
...
def phase2exp(phase, output_phase_format=DEFAULT_EXTERNAL_PHASE_REP_FORMAT, roundit=True, same_type=True):
    ...
    if is_scalar(phase) and same_type:
        scalar = True
    else:
        scalar = False
...
def r_phase2exp(phase, output_phase_format):
    ...
    try:
        ...
        raise QiskitError("phases must be complex numbers in ``[1, -1j, -1, 1j]")
...
def exp2phase(phase_exponent, input_phase_format, same_type=True):
    ...
    if is_scalar(phase_exponent) and same_type:
        scalar = True
    else:
        scalar = False
    ...
    if not is_exp_type(phase_exponent, input_phase_format):
        raise QiskitError(f"{phase_exponent} not in {input_format} format")
...
def r_str2phase(phase_string, scalar=False):
    """Convert the string representing the phase of a Pauli to a complex type phase
...
def from_label(label, qubit_order="right-to-left"):
    ...
    Args:
        ...
        qubit_order: the order in which qubits are assigned to Paulis in the string.
...
def indices_to_boolean(indices, dim):
    ...
    Args:
        indices (iterable object): Non negative integers (indices)
