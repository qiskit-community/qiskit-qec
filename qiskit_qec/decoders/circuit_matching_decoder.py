...
class CircuitModelMatchingDecoder(ABC):
    ...
    def __init__(
        ...
        self.z_gauge_products = temp_gauge_products(
            self.css_z_stabilizer_ops, self.css_z_gauge_ops
        )
        self.x_gauge_products = temp_gauge_products(
        ...
        if not self.uniform:
            fe = FaultEnumerator(
                circuit, order=1, method="propagator", model=self.model
            )
    ...
    def _process_graph(
        ...
        Dict[Tuple[int, List[int]], int], List[List[int]], DecodingGraph, List[str]
    ]:
                ...
                if source["time"] == target["time"]:
                    if source["is_boundary"] and target["is_boundary"]:
                        if source["qubits"] != target["qubits"]:
                            ...
                            edge["qubits"] = list(
                                set(source["qubits"]).intersection(
                                    (set(target["qubits"]))
                                )
                            )
    ...
    def update_edge_weights(self, model: PauliNoiseModel):
        ...
        """
        parameter_values = [
            model.get_error_probability(name) for name in self.parameters
        ]
        if not self.uniform:
            ...
            for source, target in self.graph.edge_list():
                ...
                if "weight_poly" in edge_data:
                    ...
                    restriction = {
                        x: assignment[x] for x in edge_data["weight_poly"].gens
                    }
    ...
    def _enumerate_events(
        ...
        Tuple[int, Tuple[int]], Dict[Tuple[int, Tuple[int]], Dict[List[str], int]]
    ]:
        ...
        for event in fault_enumerator.generate():
            ...
            if len(highlighted) > 2:
                raise QiskitQECError(
                    "too many highlighted vertices for a " + "single fault event"
                )
            if len(highlighted) == 1:  # _highlighted_vertices highlights the boundary
                raise QiskitQECError(
                    "only one highlighted vertex for a " + "single fault event"
                )
            if len(highlighted) == 2:
                v0 = highlighted[0]
                v1 = highlighted[1]
                ...
                for name in self.model.get_operations():
                    if name in event_map[n1][n2]:
                        for pauli, count in event_map[n1][n2][name].items():
                            expr += (
                                count
                                * model.get_pauli_weight(name, pauli)
                                * symbs[name]
                            )
