...
class HHCDecoder(CircuitModelMatchingDecoder):
    ...
    def __init__(
        ...
        for step in self.round_schedule:
            if step == "z":
                ...
                self.bits_per_round += len(css.x_gauge_ops)
        super().__init__(
            css_x_gauge_ops,
    ...
    def _partition_outcomes(self, blocks: int, round_schedule: str, outcome: List[int]):
        ...
        final_outcomes = outcome[-self.code.n :]  # TODO: FIX
        # Process the flags
    ...
    def _process_flags(
        ...
        """
        frame = self.code.n * [0]  # Z error frame  FIX FIX FIX
        ...
        for r in range(blocks):
            for s in range(len(round_schedule)):
                ...
                if round_schedule[s] == "x":
                    ...
                    syn = self.code._syndrome(frame, self.css_x_gauge_ops)
