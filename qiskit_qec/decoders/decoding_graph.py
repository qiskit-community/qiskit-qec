                    ...
                    for source in nodes:
                        for target in nodes:
                            if target != source or (len(nodes) == 1):
                                ...
                                qubits = []
                                if not (
                                    source["is_boundary"] and target["is_boundary"]
                                ):
                                    qubits = list(
        ...
        # generally applicable but approximate method
            ...
            for n0, n1 in self.graph.edge_list():
                ...
                if self.graph[n0]["is_boundary"] and not self.graph[n1]["is_boundary"]:
                    ...
                    not self.graph[n0]["is_boundary"] and self.graph[n1]["is_boundary"]
                ):
                    edge = (n0, n0)
                else:
                    edge = (n0, n1)
        ...
        for (n0, n1), prob in error_probs.items():
            ...
            if n0 != n1:
                ...
                if qubits:
                    ...
                    assert (
                        node0["time"] == node1["time"]
                        and node0["qubits"] != node1["qubits"]
                    ) or (
                        node0["time"] != node1["time"]
                        and node0["qubits"] != node1["qubits"]
                    )
                        ...
                        for node in [node0, node1]:
                            ...
                            for dt, pairs in enumerate(self.code.schedule):
                                if pair in pairs:
                                    dts.append(dt)
                        time = [
                            max(0, node0["time"] - 1 + (max(dts) + 1) / round_length)
                        ]
                    ...
                    time = [
                        node0["time"],
                        node0["time"] + (round_length - 1) / round_length,
                    ]
                    time.sort()
            else:
    ...
    def _decoding_graph(self):
        ...
        for time, layer in enumerate(self.layer_types):
            ...
            for i in range(len(boundary) - 1):
                ...
                graph.add_edge(
                    idxmap[(time, tuple(bound_g))], idxmap[(time, tuple(bound_h))], edge
                )
                logging.debug(
                    "spacelike boundary t=%d (%s, %s)", time, bound_g, bound_h
                )
