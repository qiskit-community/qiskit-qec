...
class RepetitionCodeCircuit:
    ...
    def syndrome_measurement(
        self, final: bool = False, barrier: bool = False, delay: int = 0
    ):
    ...
    def string2nodes(self, string, **kwargs):
        ...
        separated_string = self._separate_string(
            string
        )  # [ <boundary>, <syn>, <syn>,...]
        nodes = []
...
class ArcCircuit:
    ...
    def __init__(
        ...
        if run_202:
            ...
            if num_links > 0:
                ...
                self.roundbuffer = np.ceil(
                    (self.rounds_per_link - self.rounds_per_202) / 2
                )
    ...
    def _scheduling(self):
        ...
        while aux:
            ...
            matching = [
                rx.max_weight_matching(graph, max_cardinality=True, weight_fn=weight_fn)
            ]
            ...
            matching.append(
                rx.max_weight_matching(
                    cut_graph, max_cardinality=True, weight_fn=weight_fn
                )
            )
    ...
    def _preparation(self):
        ...
        for basis in list({self.basis, self.basis[::-1]}):
            self.circuit[basis] = QuantumCircuit(
                self.link_qubit, self.code_qubit, name=basis
            )
            if self.logical == "1":
        ...
        if self.run_202 and int(t / self.rounds_per_link) < len(self.links_202):
            ...
            for n in ns:
                ...
                qubit_l_nghbrs.append(
                    [
                        graph.get_edge_data_by_index(ngbhr)["link qubit"]
                        for ngbhr in neighbors
                    ]
                )
                ...
                if tau == (self.rounds_per_202 - 1):
                    ...
                    for j in range(2):
                        if qubit_l_nghbrs[j]:
                            ...
                            control_link = self.links[
                                self.link_index[qubit_l_nghbrs[j][0]]
                            ]
                            # find the qubit on which it overlaps with the 202
                            qubit_t = list(
                                set(target_link).intersection(set(control_link))
                            )[0]
                            ...
                            qc.x(self.code_qubit[q_t]).c_if(
                                self.link_bits[self.T][q_c], 1
                            )
        ...
        for syn_type, _ in enumerate(separated_string):
            for syn_round in range(len(separated_string[syn_type])):
                ...
                for elem_num, element in enumerate(elements):
                    if (
                        syn_type == 0 and (all_logicals or element != self.logical)
            ...
            for qc in circuits:
                ...
                for gate in qc.data:
                    if gate[0].name == "delay":
                        ...
                        total_delay[0][q] += t
                        new_t = 16 * np.ceil(
                            (total_delay[0][q] - total_delay[1][q]) / 16
                        )
