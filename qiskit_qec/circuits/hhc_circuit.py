...
class HHCCircuit:
    ...
    def __init__(
        ...
        self.iinitlabel = "idinit"  # label for idle in the beginning of circuit to add initialization error
    ...
    def _validate(
        ...
        if len(self.logical_paulis) > 0 and len(self.logical_paulis) != len(
            round_schedule
        ):
        ...
        if self.num_initialize is not None:
            if self.num_initialize < 0:
                raise Exception(
                    "expected zero or positive integer number of initialization"
                )
    ...
    def _hex_ancillas(self, code: HHC) -> Tuple[int, List[List[int]], List[List[int]]]:
        ...
        for j, zg in enumerate(code.z_gauges):
            if len(zg) == 2:  # boundary Z gauge
                ...
                start = total_ancilla
                z_ancilla_indices.append(
                    [code.n + start, code.n + start + 1, code.n + start + 2]
                )
                total_ancilla += 3
            else:  # bulk Z gauge
        ...
        for i in range(len(self.code.x_gauges)):
            for j in self.x_ancilla_indices[i]:
                ...
                if not group_meas:
                    if ibm:
                        circ.append(
                            IGate(label=self.ibmlabel + str(cbits[i])), [self.qreg[j]]
                        )
            ...
            for j, zg in enumerate(self.code.z_gauges):
                ...
                if step == 0:  # done in previous steps, no idles
                    ...
                    if group_meas:
                        ...
                        if ibm:
                            circ.append(
                                IGate(label=self.ibmlabel + str(rflags[j])), [right]
                            )
                    ...
                    if group_meas:
                        ...
                        if ibm:
                            circ.append(
                                IGate(label=self.ibmlabel + str(cbits[j])), [middle]
                            )
                            circ.append(
                                IGate(label=self.ibmlabel + str(lflags[j])), [left]
                            )
    ...
    def syndrome_measurement(
        ...
        round_schedule = (
            self.round_schedule if round_schedule is None else round_schedule
        )
        basis = self.basis if basis is None else basis
        ...
        if add_barriers:
            if (
                num_initialize is None
                or basis == "x"
                or (basis == "z" and initial_state == "-")
            ):
                circ.barrier(self.qreg)
