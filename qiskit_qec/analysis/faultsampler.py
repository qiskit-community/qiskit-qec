...
class FaultSampler:
    ...
    def __init__(
        ...
        self.faulty_ops_labels = [
            self._node_name_label(x[0]) for x in self.faulty_nodes
        ]
        ...
        self.label_to_error_probability = {
            label: self.model.get_error_probability(label)
            for label in self.faulty_ops_labels
        }
    ...
    def _faulty_circuit(
        self, comb: Tuple[DAGNode], error: Tuple[str]
    ) -> QuantumCircuit:
    ...
    def sample_one(self):
        ...
        uniform_samples = np.random.uniform(
            low=0.0, high=1.0, size=len(self.faulty_nodes)
        )
