...
Jul 27, 2021 Drew Vandeth
    ...
    Moved _from_label, _split_pauli_label, _phase_from_complex functions to the
    PauliRep class with some functions being updated to be more general etc. See
    PauliRep Class notes. PauliRep is now a base class of BasePauli

    Removed _VALID_LABEL_PATTERN as no longer need here. A more general method
    can be found in PauliRep class.

    Added input variable to __init__: input_qubit_order: Default: "right-to-left"
    order in which input string representations are indexed for qubits.
    The internal order is not changed. The option "right-to-left" reads a pauli
    (say XZY) as Y on qubit 0, Z on qubit 1 and X and qubit 2. "left-to-right"
    reads a pauli (say XZY) as X on qubit 0, Z on qubit 1 and Y on qubit 2

August 10, 2021 Drew Vandeth

    New ability to use QubitMaps to access elements of a Pauli via labels defined
    in those Maps. For example if P=Pauli("iXYZ") then normally P[0] will return
    the Pauli attached to qubit 0 - so Z in this example. If a QubitMap is registered
    with the Pauli (instance or class) and is asigned than a different indexing can
    be used base on that Map. For example: If consider the QubitMap that has
    (0,0)->0, (0,1)->1, (1,0)->2 and (1,1)->3. Then for the Pauli P=Pauli("XYZI")
    then P[(0,1)]=Z etc.

    __getitem__ and __call__ methods are used to enable this functionality. The
    __call__ method us used to directly access the a registered and assigned
    QubitMap's internal function method if one is available. A function method
    is used to calculate the QubitMap rather than via a lookup table if set. Thus
    P(2) will explicitly try to use the function method if it exists. The
    __getitem__ method will use either the function method or lookup method of the
    associated QubitMap.

    See TranslationMaps for more details on QubitMaps and TranslationMaps in
    general

    To use a QubitMap it must first be registered. This can be done in two
    ways - registering with the Pauli class or registering with the Pauli instance.
    Multiple QubitMaps can be registered at the same time. Registration is done
    as follows:

    B=Pauli("X1Z2Y4")
    my_map = QubitMap(data={0:4,1:3,2:2,3:1,4:0})

    Class Registration can be done by setting class_register=True within the registration
    method and likewise instance_register=False for instance registration. A QubitMap
    can be stimultaneously registered in the class and instance.

    Example: Class registeration:
    B.register(my_map, identifier=1, class_register=True, instance_register=False, override=True)

    See Pauli.register for more details

    A QubitMap can be used or assigned by using the use_instance_map or use_class_map
    methods:

    Example: For a QubitMap registered with id=1

    B.use_instance_map(1)
    B.use_class_map(1)

    Index translation can be turned of with the method use_no_map
...
from qiskit_qec.operators.paulis_base import PaulisBase
import qiskit_qec.utils.pauli_rep

from qiskit.quantum_info.operators.symplectic.qubit_map import TranslationMap
...
class Pauli(BasePauli):
    ...
    *Pauli* group
    ...
    Much of the originl document has been removed. This it should be put back and updated 
    ...
    def __init__(self, data=None, x=None, *, z=None, label=None, input_qubit_order="right-to-left"):
        ...
        Args:
            ...
            input_qubit_order: Default: "right-to-left" Order in which string representations are indexed
                for qubits on input. "right-to-left" reads a pauli (say XZY) as Y on qubit 0, Z on qubit 1
                and X and qubit 2. "left-right-order" reads a pauli (say XZY) as X on qubit 0, Z on qubit 1
                and Y on qubit 2
        Raises:
        ...
        if isinstance(data, BasePauli):
            ...
            base_z, base_x, base_phase = self._from_label(data, qubit_order = input_qubit_order)
        ...
        # Initialize the instance map dictionary
        self.__instance_map_dict__ = dict()
        self.qubitmap = None
        self.__instance_getitem__ = self.__getitem_nomap__
    ...
    def phase(self):
        ...
        return PauliRep.change_representation(self._phase, self._count_y())
    ...
    # TranslationMaps/QubitMaps: Access individual Pauli Operators on qubits
    #
    # The __call__ and __getitem__ methods are used to access the individual
    # Pauli operators for individual or a selction of qubits.
    #
    # The __getitem__ method points to __instance_getitem__ which in turn
    # either points to __getitem_map__ or __getitem__nomap__. If the
    # __instance_getitem__ method points to __getitem__nomap__ method then no
    # translation of indices is performed. If it points to the __getitem_map__
    # method then first a translaton of the qubit indices will be preformed and
    # then those calculated indices will be passed on the __getitem__nomap__ method
    # 
    # The methods use_class_map, use_instance_map methods and use_no_map change
    # where the __instance_getitem__ method points.
    #
    # When the __getitem__ method uses a QubitMap the QubitMap decides whether
    # to use a function calculation or table lookup for index translation.
    # Which method (function or lookup) is used within the QubitMap can be changed
    # by accessing the appropriate QubitMap method.
    #
    # If a function is registered with the QubitMap then index translation can be
    # forced to use the function method (without changing the QubitMap preference)
    # by using the __call__ method for the Pauli:
    #
    # Example: P(index)
    # Note: This overrides the __call__ method from OperatorBase class
    # Before using should check that this is not a bad thing or provide
    # and option to allow this override

    def __call__(self, qubits):
        # Change this later to give the option to have P(i)=P[i] etc
        try:
            qubits = self.qubitmap(qubits)
        except:
            raise QiskitError("No function registered with __call__ method")
        return Pauli((self.z[qubits], self.x[qubits]))

    def __getitem__(self, qubits):
        """Return the unsigned Pauli for subset of qubits."""
        return self.__instance_getitem__(qubits)

    def __getitem_map__(self, qubits):
        """__getitem__ method with index translation by via 
        an assigned QubitMap

        Args:
            qubits: A label/index for a single quibit or an array/list of
            labels/indices of qubits

        Returns:
            Pauli: Pauli operator associated with qubits
        """
        qubits = self.qubitmap[qubits]
        return self.__getitem_nomap__(qubits)

    def __getitem_nomap__(self, qubits):
        """__getitem__ method with no index translation

        Args:
            qubits: A single or list/array of qubits (non-negative integers)
    ...
    def use_class_map(self, identifier):
        """Specify that a class registered QubitMap with given identifier will
        be used for index translation

        Args:
            identifier (integer): QubitMap identifer
        """
        if not self.map_is_registered_in_class(identifier):
            QiskitError(f"Class map with identifier {identifier} is not"
                "registered in the class map dictionary")
        self.__instance_getitem__ = self.__getitem_map__
        self.qubitmap = Pauli.__class_map_dict__[identifier]

    def use_instance_map(self, identifier):
        """Specify that an instance registered QubitMap with given identifier will
        be used for index translation

        Args:
            identifier (integer): QubitMap identifer
        """
        if not self.map_is_registered_in_instance(identifier):
            QiskitError(f"Class map with identifier {identifier} is not"
                "registered in the instance map dictionary")
        self.__instance_getitem__ = self.__getitem_map__
        self.qubitmap = self.__instance_map_dict__[identifier]

    def use_no_map(self):
        """Specify that no index transaltion is to be used
        """
        self.__instance_getitem__ = self.__getitem_nomap__
        self.qubitmap = None
    ...
    def register(self, mapper, identifier=None, class_register=True, instance_register=False, override=True):
        if not isinstance(mapper, TranslationMap):
            QiskitError("Maps being registered must be of class QubitMap")
        if identifier is None:
            from random import randrange
            id_num = randrange(100000000)
            identifier = id_num
        if not isinstance(identifier, (int,np.integer)):
            QiskitError("Parameter identifier must be a non-negative integer")
        if identifier < 0:
            QiskitError("Parameter identifier must be a non-negative integer")
        if class_register:
            self._register_class_map(mapper=mapper, identifier=identifier, override=override)
        if instance_register:
            self._register_instance_map(mapper=mapper, identifier=identifier, override=override)
        return identifier

    def _register_class_map(self, *,mapper, identifier, override):
        if not override:
            if identifier in Pauli.__class_map_dict__.keys():
                QiskitError("Override is on. Mapper with same identifty in class map dictionary already" 
                    "registered. Set override=\"True\" to force override")
        Pauli.__class_map_dict__[identifier]=mapper

    def _register_instance_map(self,*,mapper, identifier, override):
        if not override:
            if identifier in self.__instance_map_dict__.keys():
                QiskitError("Override is on. Mapper with same identifty in instance map dictionary already"
                    "registered. Set override=\"True\" to force override")
        self.__instance_map_dict__[identifier]=mapper

    def map_is_registered(self, identifier):
        return (self.map_is_registered_in_class(identifier) or self.map_is_registered_in_instance(identifier))

    def map_is_registered_in_class(self, identifier):
        return (identifier in Pauli.__class_map_dict__.keys())

    def map_is_registered_in_instance(self, identifier):
        return (identifier in self.__instance_map_dict__.keys())

    def class_map_dict(self):
        return Pauli.__class_map_dict__

    def instance_map_dict(self):
        return self.__instance_map_dict__
