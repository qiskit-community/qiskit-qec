...
class TestARCCodes(unittest.TestCase):
        ...
        for basis in [code.basis, code.basis[::-1]]:
            ...
            for _, _, _, output in fault_paths:
                ...
                self.assertTrue(
                    ...
                    "Error: Single error nodes are not neutral",
                )
    ...
    def test_202s(self):
        ...
        self.assertTrue(
            nodeless, "Non-trivial nodes found for noiseless [[2,0,2]] circuits."
        )
    ...
    def test_feedforward(self):
        ...
        for j in range(2):
            ...
            for string in counts:
                ...
                correct_final = (
                    code.logical + str((int(code.logical) + 1) % 2) + code.logical * 2
                )
    ...
    def test_bases(self):
        ...
        self.assertTrue(
            rightops, "Error: Required rotations for basis changes not present."
        )
    ...
    def test_anisotropy(self):
        ...
        for j in range(1, link_num - 1):
            self.assertTrue(
                color[2 * j] != color[2 * (j - 1)]
                or color[2 * j] != color[2 * (j + 1)],
    ...
    def test_transpilation(self):
        ...
        self.assertTrue(
            circuit[code.base].count_ops()["x"] == 2,
            "Error: Wrong echo sequence for link qubits.",
        )
        circuit = code.transpile(backend, echo_num=(2, 0))
        self.assertTrue(
            circuit[code.base].count_ops()["x"] == 8,
            "Error: Wrong echo sequence for code qubits.",
        )
        self.assertTrue(
    ...
    def test_clustering_decoder(self):
        ...
        for row in [0, 1]:
            for j in range(half_d - 1):
                ...
                links_ladder.append(
                    (delta + 2 * j, delta + 2 * j + 1, delta + 2 * (j + 1))
                )
...
class TestGenericCodes(unittest.TestCase):
    ...
    def test_202(self):
        ...
        self.assertTrue(
            ...
            "Decoding graph for `GenericCodeCircuit` has"
