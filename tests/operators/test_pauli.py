...
from qiskit_qec.qec_utils.pauli_rep import split_pauli, cpxstr2exp
...
class TestPauliProperties(QiskitTestCase):
    ...
    def test_equal(self, label1, label2):
        ...
        target = (
            ...
            and pauli1.phase_exp == pauli2.phase_exp
        )
    ...
    @unpack
    def test_phase_setter(self, pauli, phase_exp):
        """Test phase setter"""
        pauli = Pauli(pauli)
...
class TestPauli(QiskitTestCase):
    ...
    @data(0, 1, 2, 3)
    def test_negate(self, phase_exp):
        """Test negate method"""
        op = Pauli(([False], [True], phase_exp))
        neg = -op
        self.assertTrue(op.equiv(neg))
        self.assertEqual(neg.phase_exp, (op.phase_exp + 2) % 4)
