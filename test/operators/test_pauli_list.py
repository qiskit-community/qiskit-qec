...
class TestPauliListInit(QiskitTestCase):
    ...
    def test_array_init(self):
        ...
        with self.subTest(msg="split bool array"):
        ...
        with self.subTest(msg="bool array no copy"):
            ...
            pauli_list = PauliList(matrix)
            matrix[0, 0] = not matrix[0, 0]
            np.testing.assert_equal(pauli_list.matrix, matrix)
    ...
    def test_string_init(self):
        ...
        with self.subTest(msg='str init "iZ"'):
            ...
            z = np.array([[True]])
            x = np.array([[False]])
            phase_exp = np.array([3])
            np.testing.assert_equal(pauli_list.z, z)
            np.testing.assert_equal(pauli_list.x, x)
            np.testing.assert_equal(pauli_list.phase_exp, phase_exp)

        with self.subTest(msg='str init "-Z"'):
            pauli_list = PauliList("-Z")
            z = np.array([[True]])
            x = np.array([[False]])
            phase_exp = np.array([2])
            np.testing.assert_equal(pauli_list.z, z)
            np.testing.assert_equal(pauli_list.x, x)
            np.testing.assert_equal(pauli_list.phase_exp, phase_exp)
...
class TestPauliListProperties(QiskitTestCase):
    ...
    def test_phase_property(self):
        """Test phase_exp property"""
        with self.subTest(msg="phase_exp"):
            pauli = PauliList(["XI", "IZ", "YY", "YI"])
            array = np.array([0, 0, 0, 0], dtype=int)
            np.testing.assert_equal(pauli.phase_exp, array)
